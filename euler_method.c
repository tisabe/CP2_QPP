...
void euler_method(double complex *out, double complex *in, parameters params) {
    ...
    for(int t=0; (t * params.tau) < params.total_time; t++){
        // Calculate H(psi)
        hamiltonian(ham_out, out, params.N, params.D, params.L, params.mhat, params.epsilon, params.ext_potential_type, params.parameter);
