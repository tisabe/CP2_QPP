...
void hamiltonian(double complex *out, long int *neighbours, double complex *in, long int N, unsigned int D, long int L, double m, double epsilon, int ext_potential_type, double parameter){
  /*Calculate the kinetic part*/
  double complex *phi_kinetic= malloc(L*sizeof(double complex));
  kinetic(phi_kinetic,neighbours,in,N,D,L,m);
