...
int main() {
  ...
  for(double N = 8; N <= pow(2,16); N *= sqrt(2)){
    params.N = (long int)N;
    params.D = 1;
    ...
    for(int i=0; i < 100; i++){
      ...
      t_start = omp_get_wtime();
      step_strang_parallel(out_wf, start_wf, params);
      t_duration = omp_get_wtime() - t_start;
    ...
    printf("Minimum time:\n");
    printf("Static nn array\tParallel\tSpeedup\n");
    printf("%e\t%e\t%e\n\n\n",reg_min,parallel_min,reg_min/parallel_min);
